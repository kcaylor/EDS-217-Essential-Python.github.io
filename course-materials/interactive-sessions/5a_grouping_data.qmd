...
Yesterday afternoon, in the last coding task of the colab, you were asked to compare the mean dissolved oxygen at two different sites. That took coding up two different filters and two `.mean()` calls. At the time, the instructions mentioned that today you would
learn how to do analyze all six sites at once.
...
average", but "what is the average **for each X**": for each site, for each species, for each year,
for each ocean. This morning we will learn the code pattern that answers all of those, and we will
write it in both of the forms you will meet in other people's code. In doing so, we will revisit commands and tools we haven't seen since the first day of class when we were doing our data science workflow demo.
...
5. Rebuild yesterday's clean stream-chemistry table. Every line below is one you wrote in yesterday afternoon's colab, collected here so you can start from a table that is clean and well-organized:
...
255 rows out of the 320 in the raw file, which should be the number you finished on yesterday. 
...
The whole-table mean is accurate, but it is also mostly useless. It is the average of six streams that are not the same stream.

Yesterday you got two of the six answers the only way you could:
...
Look carefully at the output. The grouped object is not a table. 
`.groupby()` on its own does the **split** and stops there, holding pointers to six different piles of rows. No arithmetic has happened yet, and none will happen until you name a calculation.
...
:::{.callout-note}
🐍 The object `.groupby()` returns is a real object that you can keep in a variable and reuse. Ask it for a different column and the table is not split again, because the split has already
happened:
...
You will rarely need the grouped object for its own sake, so most of the pandas code you read runs the two steps together:

```{python}
survey.groupby('site')['dissolved_oxygen_mg_L'].mean()
```

**Both forms are the same pattern.** The two-step version stores the middle of it in a variable and the one-line version does not. 

Same split, same apply, same combine, same answer.
...
Forty-five samples at `site_c` and thirty-nine at `site_d`, which is close enough to even that comparing their means is reasonable. When the counts are lopsided, `.count()` is the line that tells you so, before you have written the comparison into a report. Get in the habit of asking
for it.
...
:::{.callout-note}
🐍 `.value_counts()`, which you have used since Day 2, is a grouped count in disguise. The two lines below produce the same numbers:
...
Read the three outputs together and there is a result in them. The sites with the warmest water have the least oxygen in it, in order, without a single exception. Warm water holds less dissolved oxygen than cold water, which is a fact about gases, so we should expect to see this pattern. But it's nice to confirm that our data are showing us a pattern we expect!
