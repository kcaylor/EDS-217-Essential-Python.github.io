...
Nobody designed this dataset. It was assembled, one study at a time. Most of the environmental
data you will ever be handed was put together the same way, which is why today mattered.

You have already met this file twice today. Tonight you will take it the whole way: from the
raw download to an answer you could defend about whether the Atlantic and the Pacific are
different.
...
Everything below is built from these three patterns, plus two more you learned yesterday. You
should not need to look any of them up more than once.
...
Part 2 takes judgement rather than syntax, and more than one answer here is defensible. Every
task here removes rows, so **print `.shape` after every one of them** and record the number in
a markdown cell.
...
13. Add a column called `year` holding the year each sample was taken, using the supplied line
    ...
    across its fifty years, and what an uneven spread would mean for anyone who wants to use it
    to describe a trend.
...
On Day 1 you drew a line with `plt.plot(series)`. A histogram is the same kind of call:
...
16. The histogram you just drew is useless, and it is useless for a reason you can state in one
    sentence. State that reason in a markdown cell.
...
Somebody decided those cutoffs. Tonight you will decide your own, and then find out whether you
agree with theirs.
...
Six patterns, and two of them you also used yesterday and the day before.

Notice which one did the most work today. It was `df['new'] = expression`, four times over:
`ocean`, `year`, `log10_measurement`, and `my_class`, and none of the cleaning patterns comes
close. The twenty-two columns you downloaded did not have the answer to the journalist's
question in them, and the four columns you made yourself are where you found it!
