...
:::{.callout-tip}
I've already taken a look at this data set and noticed there was a column with sample date called `Date`. We can use the `parse_date` option of the `read_csv()` function to convert values in the `Date` column of the csv into datetime objects in pandas while reading the file. 
:::
