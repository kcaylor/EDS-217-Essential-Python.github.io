...
```markdown
# Day 7: Session A - Visualizations, Part I
...
There are extensive options for plotting in Python – some favorites include statistical visualizations in [`Seaborn`](https://seaborn.pydata.org), interactive plots for notebooks/web applications in [`Plotly`](https://plotly.com/python/), and even grammar-of-graphics approaches similar to `ggplot`, such as [Plotnine](https://plotnine.org). The original and fundamental library for visualizations in Python, however, is [<span class="codeb">matplotlib`](https://matplotlib.org). 
